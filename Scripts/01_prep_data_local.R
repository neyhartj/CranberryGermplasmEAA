...
genotyping_dir <- file.path(cran_dir, "Genotyping")
geno_dir <- file.path(genotyping_dir, "MarkerDatabase")
...
# Load the unphased, unimputed SNP data  --------------------------------------------------

vcf_unphased_file <- file.path(geno_dir, "unphased_marker_genotype_db.vcf")
vcf_unphased <- read.vcfR(file = vcf_unphased_file)

# Subset the wild samples
vcf_unphased_wild <- vcf_unphased[,c(1, which(colnames(vcf_unphased@gt) %in% wild_accessions))]

# Save this VCF
vcf_file <- file.path(data_dir, "wild_cranberry_unphased_genotypes.vcf.gz")
write.vcf(x = vcf_unphased_wild, file = vcf_file)
